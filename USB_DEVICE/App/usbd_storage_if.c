...
#define STORAGE_BLK_NBR                  SECTOR_COUNT
...
//  uint32_t len  = blk_len * SECTOR_SIZE;
//
//  /* Стираем нужные 4KB-секторы */
//  uint32_t start = addr & ~(FLASH_SECTOR_SIZE - 1);
//  uint32_t end   = addr + len;
//
//  for (uint32_t a = start; a < end; a += FLASH_SECTOR_SIZE)
//  {
//      SPI_Flash_EraseSector(a);
//  }
//
...
//  return USBD_OK;
    ...
    // НИЧЕГО НЕ ПИШЕМ ВО ФЛЕШКУ ОТ ХОСТА
    return USBD_FAIL;
  /* USER CODE END 7 */
}
