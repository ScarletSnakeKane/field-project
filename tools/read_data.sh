...
# Ловить окно бодрствования надо САМОЙ командой прошивки, а не подключением.
# Голое подключение ядро не удерживает: программатор при выходе отпускает его,
# плата тут же засыпает, и пойманный доступ пропадает. Команда записи захватывает
# ядро и держит его на всё время операции.
#
# Окно ждём долго и опрашиваем часто: при часовом интервале плата бодрствует
# около полутора секунд из трёх с половиной тысяч, так что попытка раз в секунду
# нужна, чтобы не проскочить мимо, а ждать приходится до целого часа.
FLASH_WAIT_SEC="${FLASH_WAIT_SEC:-4200}"
flash_now() {
    local end=$(( $(date +%s) + FLASH_WAIT_SEC ))
    while [ $(date +%s) -lt $end ]; do
        # grep ищет по всему потоку, разделители строк ему безразличны,
        # поэтому CR-only вывод программатора здесь преобразовывать не нужно.
        if "$PROG" -c port=SWD mode=UR -w "$ELF_W" -v -rst 2>&1 | grep -q "verified successfully"; then
            return 0
        fi
        sleep 1
    done
    echo "не удалось поймать окно бодрствования за $FLASH_WAIT_SEC с" >&2
    return 1
}
