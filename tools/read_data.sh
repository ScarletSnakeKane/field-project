...
# На плате в этот момент может лежать дампер — прошивка, которая намеренно
# висит в while(1) и ничего не пишет. Оставить её там после аварийного выхода
# значит молча остановить сбор данных, поэтому рабочая версия возвращается
# при любом исходе.
on_exit() {
    local code=$?
    restore_src
    if [ "${FLASHED_DUMPER:-0}" = "1" ] && [ "${RESTORED_FW:-0}" != "1" ]; then
        ...
        "$ROOT/tools/dev.sh" build >/dev/null 2>&1 && flash_now ||             echo "!! НЕ УДАЛОСЬ. Прошейте вручную: ./tools/dev.sh flash" >&2
    fi
    exit $code
}
trap on_exit EXIT
...
words=$(retry 20 " : " -c port=SWD mode=HOTPLUG -r32 "$MAG" 12         | grep -E "^0x[0-9A-Fa-f]+ : " | tail -1)
