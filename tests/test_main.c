...
static void test_aht20(void)
{
    ...
    /* Края шкалы: нули в ОДНОМ из полей законны и отвергаться не должны,
     * иначе прибор ослепнет на сухом воздухе и на -50 C соответственно. */
    ...
    f[0] = 0x18; f[1] = 0x80; f[2] = 0x00; f[3] = 0x00; f[4] = 0x00; f[5] = 0x00;
    equ(AHT20_DecodeFrame(f, &t, &h), AHT20_DEC_OK, "нулевая температура при живой влажности — норма");
    eqf(t, -50.0f, 0.01f, "температура -50 C как законное значение шкалы");

    /* Верхний край: все единицы в 20 битах. */
    f[0] = 0x18; f[1] = 0xFF; f[2] = 0xFF; f[3] = 0xFF; f[4] = 0xFF; f[5] = 0xFF;
    equ(AHT20_DecodeFrame(f, &t, &h), AHT20_DEC_OK, "полная шкала принята");
    eqf(h, 100.0f, 0.01f, "влажность 100 %");
    eqf(t, 150.0f, 0.01f, "температура 150 C — верх шкалы датчика");
}
